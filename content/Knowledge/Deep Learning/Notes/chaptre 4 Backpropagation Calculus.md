...
tags:
  ...
  - Neural_Network
Date /Time: "{date} {time}"
title: Blue1Brown Neural Network Serie Notes Chapter 3 Backpropagation calculus
---
...
so the cost for this simple network is $$C_0 =(a^{(L)}-y)^2$$ this is called the [[Mean Squared Error]] 
we know that $$a^{(L)} =\sigma(z^{(L)})$$ where $z^{(L)}=\omega^{(L)}a^{(L-1)}+b^{(L)})$
...
$$\frac{\partial C_0}{\partial a^{(L)}}=2(a^{(L)}-y)$$$$\frac{a^{(L)}}{\partial z^{(L)}} =\sigma'(z^{(L)})$$
$$\frac{\partial z^{(L)}}{\partial \omega^{(L)}}=a^{(L-1)}$$
so$$\frac{\partial C_0}{\partial \omega^{(L)}} =a^{(L-1)}\sigma'(z^{(L)})2(a^{(L)}-y)
$$and the cost of this simple network for all of the training data is $$\frac{\partial C}{\partial \omega^{(L)}} =\frac{1}{n}\sum_{k=0}^{n-1}\frac{\partial C_k}{\partial \omega^{(L)}}$$
...
where $C_0 = \sum_{j=0}^{n_l-1}(a_j^{(l)}-y_j)^2$
